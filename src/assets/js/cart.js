(function(){
  // Alpine.js provides a Vue-like API via Alpine.effectScope
  const storeMap = new Map();

  function useCart(storageKey = 'simplecart_cart') {
    if (!storeMap.has(storageKey)) {
      const storeRef = Alpine.effectScope(() => {
        return reactive({
          items: ref([]),
          load() {
            try { this.items.value = JSON.parse(localStorage.getItem(storageKey) || '[]'); } catch(e) { this.items.value = []; }
          },
          save() { localStorage.setItem(storageKey, JSON.stringify(this.items.value)); },
          clear() { this.items.value = []; this.save(); },
          add(p) {
            const found = this.items.value.find(i => i.product_id === p.id);
            if (found) { found.quantity += 1; } else { this.items.value.push({ product_id: p.id, name: p.name, price: p.price, quantity: 1 }); }
            this.save();
          },
          inc(i) {
            if (i.quantity >= 1000) return;
            i.quantity += 1;
            this.save();
          },
          dec(i) { i.quantity = Math.max(1, i.quantity - 1); this.save(); },
          remove(i) { this.items.value = this.items.value.filter(x => x !== i); this.save(); },
          total() { return this.items.value.reduce((s, i) => s + i.price * i.quantity, 0); }
        });
      });
      storeMap.set(storageKey, storeRef()); // Call the function to get the reactive object
    }
    const store = storeMap.get(storageKey);
    return { items: () => store.items.value, add: store.add, inc: store.inc, dec: store.dec, remove: store.remove, total: store.total };
  }
  ...
  function boot(opts){
    ...
    // Alpine.js reactive state (not stored in map so it re-creates on each mount)
    const products = ref([]);
    ...
    const app = Alpine.data('simplecartApp', () => ({
      products: computed(() => storeMap.get('simplecart_cart')?.items.value || []),
      cartItems: () => storeMap.get('simplecart_cart')?.items.value || [],
      addToCart: (p) => storeMap.get('simplecart_cart').add(p),
      inc: (i) => storeMap.get('simplecart_cart').inc(i),
      dec: (i) => storeMap.get('simplecart_cart').dec(i),
      removeItem: (i) => storeMap.get('simplecart_cart').remove(i),
      total: computed(() => storeMap.get('simplecart_cart')?.total() || 0),
